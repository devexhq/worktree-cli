...
`tests/core/test_db.py`). Test classes must be named `Test*` or `*Tests` per
`python_classes` in [pyproject.toml](../../pyproject.toml); plain `test_*` functions
work too.
...
commands at it. See [tests/commands/init/test_command.py](../../tests/commands/init/test_command.py)
