"""Small stateless helpers shared by the loop iteration step functions.
...
Stateful per-run data lives in ``steps.py`` (``_LoopContext``); the top-level
orchestration lives in ``runner.py`` (``run_loop_iteration``).
"""
