"""Seed packaged starter workflow definitions into a worktree workflows directory."""
...
class WorkflowSeedResult(BaseModel):
    """Outcome of seeding starter workflow files."""
...
def _template_root() -> resources.abc.Traversable:
    return resources.files("getworktree.core.templates.workflows")
...
def _load_and_validate_template(template_name: str) -> dict[str, Any]:
    ...
    validation = WORKFLOW_VALIDATOR.validate(parsed)
...
def seed_starter_workflows(workflows_dir: Path, force: bool = False) -> WorkflowSeedResult:
    ...
    try:
        ...
        result.errors.append(f"Could not create {display_path(workflows_dir)}: {exc}")
        return result
