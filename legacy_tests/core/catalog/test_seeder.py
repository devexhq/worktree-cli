...
class SeedCatalogTemplatesTests:
    ...
    def test_creates_missing_files(self, fs: FileSystem) -> None:
        result = seed_catalog_templates(CatalogItemType.BLUEPRINT, path=fs.base_path)
        ...
        target_dir = fs.base_path / ".worktree" / "catalog" / "blueprints" / "wt"
    ...
    def test_force_overwrites_existing_files(self, fs: FileSystem) -> None:
        ...
        result = seed_catalog_templates(CatalogItemType.BLUEPRINT, path=fs.base_path, force=True)
    ...
    def test_blueprint_seed_includes_curated_wt_files(self, fs: FileSystem) -> None:
        result = seed_catalog_templates(CatalogItemType.BLUEPRINT, path=fs.base_path)
...
class SeedAllCatalogTemplatesTests:
    ...
    def test_aggregates_all_three_types(self, fs: FileSystem) -> None:
        ...
        blueprints_dir = fs.base_path / ".worktree" / "catalog" / "blueprints" / "wt"
    ...
    def test_force_overwrites_across_all_types(self, fs: FileSystem) -> None:
        ...
        (blueprints_dir / "fix-tests.yml").write_text("custom\n", encoding="utf-8")
