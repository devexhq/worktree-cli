...
class CatalogScannerAndIndexTests:
    """Unit tests for file system scanning and database catalog index synchronization."""
    ...
    def test_catalog_db_list_by_name(self, fs: FileSystem) -> None:
        ...
        workflow_duplicates = self.db.catalog.list_by_name("dup", item_type=CatalogItemType.WORKFLOW)
...
class CatalogCrudOperationsTests:
    """Unit tests for item creation, resolution, retrieval, and deletion in the catalog."""
    ...
    def test_create_catalog_item_default_content_is_non_empty_and_unnamed(self, fs: FileSystem, item_type: str) -> None:
        ...
        content = (fs.base_path / ".worktree" / f"catalog/{item_type}s/my-blueprint.yml").read_text(encoding="utf-8")
