"""Tests for `getworktree.core.loops.metadata`."""
...
class ParseLoopMetadataTests:
    """Tests for parse_loop_metadata statuses and field codes."""
    ...
    def test_ok_on_packaged_fix_tests_template(self, tmp_path: Path) -> None:
        ...
        result = parse_loop_metadata(path)

        assert result.status == LoopMetadataStatus.OK
        assert result.ok
    ...
    def test_ok_with_extra_keys_and_incomplete_body(self, tmp_path: Path) -> None:
        ...
        assert result.ok
        assert result.metadata is not None
        assert result.metadata.name == "minimal-loop"
    ...
    def test_not_found(self, tmp_path: Path) -> None:
        ...
        assert result.status == LoopMetadataStatus.NOT_FOUND
        assert not result.ok
        assert result.metadata is None
        assert any("LOOP_META_NOT_FOUND" in error for error in result.errors)

    def test_not_a_file(self, tmp_path: Path) -> None:
        path = tmp_path / "loops-dir"
        path.mkdir()

        result = parse_loop_metadata(path)

        assert result.status == LoopMetadataStatus.NOT_A_FILE
        assert not result.ok
        assert any("LOOP_META_NOT_A_FILE" in error for error in result.errors)
    ...
    def test_unreadable(self, tmp_path: Path) -> None:
        ...
        assert result.status == LoopMetadataStatus.UNREADABLE
        assert not result.ok
        assert any("LOOP_META_UNREADABLE" in error for error in result.errors)
    ...
    def test_malformed_yaml(self, tmp_path: Path) -> None:
        ...
        assert result.status == LoopMetadataStatus.MALFORMED_YAML
        assert not result.ok
        assert any("LOOP_META_MALFORMED_YAML" in error for error in result.errors)
    ...
    def test_root_not_mapping_null_and_list(self, tmp_path: Path) -> None:
        ...
        empty_result = parse_loop_metadata(empty)
        list_result = parse_loop_metadata(listed)

        assert empty_result.status == LoopMetadataStatus.ROOT_NOT_MAPPING
        assert list_result.status == LoopMetadataStatus.ROOT_NOT_MAPPING
        assert any(
            "LOOP_META_ROOT_NOT_MAPPING" in error for error in empty_result.errors
        )
        assert any(
            "LOOP_META_ROOT_NOT_MAPPING" in error for error in list_result.errors
        )
    ...
    def test_collects_all_missing_field_codes(self, tmp_path: Path) -> None:
        ...
        assert result.status == LoopMetadataStatus.INVALID_METADATA
        joined = "\n".join(result.errors)
        assert "LOOP_META_MISSING_VERSION" in joined
        assert "LOOP_META_MISSING_NAME" in joined
        assert "LOOP_META_MISSING_DESCRIPTION" in joined
    ...
    def test_invalid_version_variants(self, tmp_path: Path) -> None:
        ...
        for index, text in enumerate(cases):
            ...
            result = parse_loop_metadata(path)
            assert result.status == LoopMetadataStatus.INVALID_METADATA
            assert any("LOOP_META_INVALID_VERSION" in error for error in result.errors)
    ...
    def test_invalid_name_variants(self, tmp_path: Path) -> None:
        ...
        for index, text in enumerate(cases):
            ...
            result = parse_loop_metadata(path)
            assert result.status == LoopMetadataStatus.INVALID_METADATA
            assert any("LOOP_META_INVALID_NAME" in error for error in result.errors)
    ...
    def test_invalid_description_variants(self, tmp_path: Path) -> None:
        ...
        for index, text in enumerate(cases):
            ...
            assert result.status == LoopMetadataStatus.INVALID_METADATA
            assert any(
                "LOOP_META_INVALID_DESCRIPTION" in error for error in result.errors
            )
