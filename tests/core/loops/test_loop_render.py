"""Tests for pure loop show text formatters."""
...
def _sample_loop(**overrides: object) -> LoopDefinition:
    raw: dict = {
        "version": 1,
        "name": "fix-tests",
    ...
    return LoopDefinition.model_validate(raw)


class FormatLoopShowSuccessTests:
    """Normative success layout for format_loop_show_success."""

    def test_full_layout_matches_template_shape(self, tmp_path: Path) -> None:
        source = (tmp_path / "fix-tests.yml").resolve()
        text = format_loop_show_success(_sample_loop(), source_path=source)
    ...
    def test_args_and_lists_and_bool_null(self, tmp_path: Path) -> None:
        ...
        loop = LoopDefinition.model_validate(data)
        source = (tmp_path / "x.yml").resolve()

        text = format_loop_show_success(loop, source_path=source)
    ...
    def test_warnings_section_and_status_token(self, tmp_path: Path) -> None:
        source = (tmp_path / "fix-tests.yml").resolve()
        text = format_loop_show_success(
            _sample_loop(),
            source_path=source,
            warnings=[
                "Duplicate loop name 'fix-tests' (LOOP_RESOLVE_DUPLICATE_NAME).",
                "second\ncontinued",
            ],
        )
        ...
        assert lines[5].startswith("- Duplicate loop name")
...
class FormatLoopShowFailureTests:
    ...
    def test_resolve_failure_fallback(self, tmp_path: Path) -> None:
        ...
        assert format_loop_show_resolve_failure(result) == "Failed to resolve loop."
    ...
    def test_validate_failure_joins_errors(self, tmp_path: Path) -> None:
        ...
        assert format_loop_show_validate_failure(result) == "schema bad\n\nmore"
    ...
    def test_validate_failure_fallback(self, tmp_path: Path) -> None:
        ...
        assert (
            format_loop_show_validate_failure(result) == "Loop definition is invalid."
        )
