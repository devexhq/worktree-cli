...
def test_effective_terminal_policy_retry_uses_on_max_retries() -> None:
...
def test_effective_terminal_policy_non_retry_returns_action() -> None:
    spec = FailureSpec(action=FailurePolicy.ABORT)
    assert effective_terminal_policy(spec) == FailurePolicy.ABORT


def test_mark_continued_after_prompt_sets_ignored_and_marker() -> None:
    result = StepResult(
        step_id="s1",
        status="failed",
        exit_code=1,
...
def test_step_failure_diagnostic_prefers_error_message() -> None:
    result = StepResult(
        step_id="s1",
        status="failed",
        exit_code=7,
