...
class TestConditionParser:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param("steps.unit.exit_code == 0", "steps.unit.exit_code", "==", "0", id="eq_operator"),
            pytest.param("steps.eval.score >= 0.85", "steps.eval.score", ">=", "0.85", id="gte_operator"),
            pytest.param(
                "steps.check.output contains success", "steps.check.output", "contains", "success", id="contains_word"
            ),
            pytest.param(
                "steps.check.output contains 'hello world'",
                "steps.check.output",
                "contains",
                "'hello world'",
                id="contains_single_quoted",
            ),
            pytest.param(
                'steps.check.output contains "hello world"',
                "steps.check.output",
                "contains",
                '"hello world"',
                id="contains_double_quoted",
            ),
            pytest.param("iteration.index > 3", "iteration.index", ">", "3", id="gt_operator"),
            pytest.param("steps.build.status != failed", "steps.build.status", "!=", "failed", id="neq_operator"),
            pytest.param("steps.test.exit_code <= 1", "steps.test.exit_code", "<=", "1", id="lte_operator"),
            pytest.param("steps.test.exit_code < 2", "steps.test.exit_code", "<", "2", id="lt_operator"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param("steps.unit.exit_code", id="missing_operator_and_right"),
            pytest.param("steps.unit.exit_code ==", id="missing_right_operand"),
            pytest.param("== 0", id="missing_left_operand"),
            pytest.param("steps.unit.exit_code === 0", id="unsupported_triple_equals"),
            pytest.param("steps.unit.exit_code in ['a', 'b']", id="unsupported_in_operator"),
            pytest.param("steps.unit.exit_code ~ 0", id="unsupported_tilde_operator"),
        ],
    )
...
class TestParseLiteral:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param("42", 42, id="positive_int"),
            pytest.param("-10", -10, id="negative_int"),
            pytest.param("3.2", 3.2, id="positive_float"),
            pytest.param("-0.85", -0.85, id="negative_float"),
            pytest.param("1e-4", 0.0001, id="scientific_float"),
            pytest.param("'hello'", "hello", id="single_quoted_string"),
            pytest.param('"hello"', "hello", id="double_quoted_string"),
            pytest.param("'it\\'s fine'", "it's fine", id="escaped_quote_string"),
            ...
            pytest.param("completed", "completed", id="status_completed"),
            pytest.param("failed", "failed", id="status_failed"),
            pytest.param("nan", "nan", id="literal_nan"),
            pytest.param("inf", "inf", id="literal_inf"),
            pytest.param("-inf", "-inf", id="literal_neg_inf"),
            pytest.param("hello world", "hello world", id="unquoted_multi_word"),
            pytest.param("", "", id="empty_string"),
        ],
    )
