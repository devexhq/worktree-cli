...
def test_construct_from_definition_does_not_copy(fs: FileSystem) -> None:
...
def test_load_from_definition_does_not_revalidate_or_interpolate() -> None:
...
def test_load_from_dict_validates_without_interpolating() -> None:
...
def test_load_from_dict_missing_shape_raises_validation_error() -> None:
    with pytest.raises(StepValidationError, match="validation failed"):
        Step.load({"id": "broken"})


def test_load_from_dict_invalid_field_type_raises_validation_error() -> None:
    with pytest.raises(StepValidationError, match="validation failed"):
        Step.load({"id": "broken", "type": "command", "command": 12})


def test_load_from_catalog_name(fs: FileSystem) -> None:
...
def test_load_from_catalog_sha(fs: FileSystem) -> None:
    fs.write_file(".worktree/catalog/steps/echo-msg.yml", _command_payload())
...
def test_load_from_catalog_uses_process_cwd_when_omitted(fs: FileSystem, monkeypatch: pytest.MonkeyPatch) -> None:
    fs.write_file(".worktree/catalog/steps/echo-msg.yml", _command_payload())
...
def test_load_unknown_catalog_name_raises_not_found(fs: FileSystem) -> None:
...
def test_load_task_only_name_raises_not_found(fs: FileSystem) -> None:
...
def test_load_malformed_catalog_yaml_raises_validation_error(fs: FileSystem) -> None:
...
def test_load_non_object_catalog_yaml_raises_validation_error(fs: FileSystem) -> None:
...
def test_load_catalog_payload_that_fails_step_validation(fs: FileSystem) -> None:
    fs.write_file(".worktree/catalog/steps/incomplete.yml", "id: incomplete\nname: incomplete\n")
    with pytest.raises(StepValidationError, match="validation failed"):
        Step.load("incomplete", catalog=Catalog(fs.base_path))


def test_dump_does_not_interpolate() -> None:
    step = Step.load(_command_payload())
...
def test_execute_interpolates_inputs_and_leaves_handle_untouched(fs: FileSystem) -> None:
    step = Step.load(_command_payload())
...
def test_execute_with_empty_inputs_does_not_interpolate(fs: FileSystem) -> None:
...
def test_execute_missing_sandbox_matches_execute_step(fs: FileSystem) -> None:
