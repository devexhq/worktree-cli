...
class TaskModelValidationTests:
    """Unit tests for TaskDefinition validation, inputs, and failure defaults."""

    def test_task_definition_parses_full_fields(self) -> None:
    ...
    def test_task_definition_defaults_use_sandbox_true(self) -> None:
    ...
    def test_task_defaults_on_failure_omitted_keeps_step_abort(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "no-defaults",
    ...
    def test_task_defaults_on_failure_inherited_when_step_omits(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "inherit",
    ...
    def test_task_defaults_on_failure_object_form_inherited(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "retry-default",
                "defaults": {
    ...
    def test_task_explicit_step_on_failure_wins_unchanged(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "explicit-wins",
                "defaults": {"on_failure": "continue"},
    ...
    def test_task_defaults_reject_unknown_keys(self) -> None:
    ...
    def test_task_defaults_on_failure_invalid_action_rejected(self) -> None:
        with pytest.raises(ValidationError):
            TaskDefinition.model_validate(
                {
                    "name": "bad-action",
    ...
    def test_task_defaults_on_failure_invalid_bounds_rejected(self) -> None:
        with pytest.raises(ValidationError):
            TaskDefinition.model_validate(
                {
                    "name": "bad-bounds",
    ...
    def test_task_definition_parses_inputs(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "commit",
                "inputs": {
...
class TaskModelShorthandTests:
    """Unit tests for task step shorthand expansions and slugified step IDs."""

    def test_fill_step_shorthand_command_maps_to_run(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "pytest-task",
    ...
    def test_fill_step_shorthand_slugifies_name_for_id(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "named-steps",
    ...
    def test_fill_step_shorthand_preserves_explicit_id_and_type(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "explicit",
                "steps": [
    ...
    def test_fill_step_shorthand_indexes_multiple_anonymous_steps(self) -> None:
        task = TaskDefinition.model_validate(
            {
                "name": "multi",
                "steps": [
