...
class ResolveWorkflowsDirTests:
    ...
    def test_explicit_absolute_path_wins(self, tmp_path: Path) -> None:
        explicit = tmp_path / "custom" / "workflows"
        resolved, errors = resolve_workflows_dir(cwd=tmp_path, workflows_dir=explicit)
        assert errors == []
    ...
    def test_default_without_config(self, tmp_path: Path) -> None:
        resolved, errors = resolve_workflows_dir(cwd=tmp_path, use_config=False)
        assert errors == []
        assert resolved == (tmp_path / DEFAULT_WORKFLOWS_DIR).resolve()

    def test_uses_config_paths_workflows_dir(self, tmp_path: Path) -> None:
        ...
        resolved, errors = resolve_workflows_dir(cwd=tmp_path)
        assert errors == []
        assert resolved == (tmp_path / "custom-workflows").resolve()

    def test_config_unavailable_when_missing(self, tmp_path: Path) -> None:
        resolved, errors = resolve_workflows_dir(cwd=tmp_path, use_config=True)
        assert resolved == (tmp_path / DEFAULT_WORKFLOWS_DIR).resolve()
        assert len(errors) == 1
...
class DiscoverWorkflowFilesTests:
    ...
    def test_ok_empty_directory(self, tmp_path: Path) -> None:
        ...
        result = discover_workflow_files(cwd=tmp_path, workflows_dir=workflows_dir)
    ...
    def test_not_found(self, tmp_path: Path) -> None:
        workflows_dir = tmp_path / "missing-workflows"
    ...
    def test_config_unavailable(self, tmp_path: Path) -> None:
        result = discover_workflow_files(cwd=tmp_path, use_config=True)
        ...
        assert result.workflows_dir == (tmp_path / DEFAULT_WORKFLOWS_DIR).resolve()
    ...
    def test_explicit_override_does_not_require_config(self, tmp_path: Path) -> None:
        workflows_dir = tmp_path / "only-workflows"
        workflows_dir.mkdir()
    ...
    def test_uses_config_when_no_explicit_dir(self, tmp_path: Path) -> None:
        ...
        _write_config(config_path, raw)
        workflows_dir = tmp_path / "from-config"
        workflows_dir.mkdir()
        (workflows_dir / "one.yaml").write_text("1\n", encoding="utf-8")

        result = discover_workflow_files(cwd=tmp_path)
