...
class ParseWorkflowMetadataTests:
    ...
    def test_ok_on_packaged_fix_tests_template(self, fs: FileSystem) -> None:
    ...
    def test_ok_on_packaged_review_fix_template(self, fs: FileSystem) -> None:
    ...
    def test_ok_with_extra_keys_and_incomplete_body(self, fs: FileSystem) -> None:
        path = _write(
            fs.base_path / "minimal.yml",
    ...
    def test_not_a_file(self, fs: FileSystem) -> None:
        path = fs.base_path / "workflows-dir"
        path.mkdir()
    ...
    def test_unreadable(self, fs: FileSystem) -> None:
        path = _write(fs.base_path / "secret.yml", "version: 1\n")
        path.chmod(0)
        try:
            result = parse_workflow_metadata(path)
        finally:
    ...
    def test_malformed_yaml(self, fs: FileSystem) -> None:
        path = _write(fs.base_path / "broken.yml", "version: [\n")
    ...
    def test_root_not_mapping_null_and_list(self, fs: FileSystem) -> None:
        empty = _write(fs.base_path / "empty.yml", "")
        listed = _write(fs.base_path / "list.yml", "- a\n- b\n")
    ...
    def test_collects_all_missing_field_codes(self, fs: FileSystem) -> None:
        path = _write(fs.base_path / "blank.yml", "other: 1\n")
    ...
    def test_invalid_version_variants(self, fs: FileSystem) -> None:
        cases = (
            "version: 2\nname: ok-name\ndescription: d\n",
            "version: 99\nname: ok-name\ndescription: d\n",
            "version: true\nname: ok-name\ndescription: d\n",
        ...
        for index, text in enumerate(cases):
            path = _write(fs.base_path / f"bad-version-{index}.yml", text)
            result = parse_workflow_metadata(path)
            assert result.status == WorkflowMetadataStatus.INVALID_METADATA
            assert any("WORKFLOW_META_INVALID_VERSION" in error for error in result.errors)

    def test_invalid_name_variants(self, fs: FileSystem) -> None:
        cases = (
            'version: 1\nname: "Fix Tests"\ndescription: d\n',
            'version: 1\nname: ""\ndescription: d\n',
            "version: 1\nname: 12\ndescription: d\n",
        ...
        for index, text in enumerate(cases):
            path = _write(fs.base_path / f"bad-name-{index}.yml", text)
            result = parse_workflow_metadata(path)
            assert result.status == WorkflowMetadataStatus.INVALID_METADATA
            assert any("WORKFLOW_META_INVALID_NAME" in error for error in result.errors)

    def test_invalid_description_variants(self, fs: FileSystem) -> None:
        cases = (
            'version: 1\nname: ok-name\ndescription: ""\n',
            "version: 1\nname: ok-name\ndescription: 1\n",
        )
        for index, text in enumerate(cases):
            path = _write(fs.base_path / f"bad-desc-{index}.yml", text)
            result = parse_workflow_metadata(path)
            assert result.status == WorkflowMetadataStatus.INVALID_METADATA
            assert any("WORKFLOW_META_INVALID_DESCRIPTION" in error for error in result.errors)

    def test_description_allows_leading_trailing_spaces(self, fs: FileSystem) -> None:
        path = _write(
            fs.base_path / "spaces.yml",
            "version: 1\nname: spaced\ndescription: ' padded '\n",
        )
