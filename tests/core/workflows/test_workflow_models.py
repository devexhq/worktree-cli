...
class WorkflowModelDefaultsTests:
    """Unit tests for WorkflowDefinition defaults propagation and override semantics."""

    def test_workflow_defaults_omitted_keeps_step_abort(self) -> None:
    ...
    def test_workflow_defaults_on_failure_inherited_when_step_omits(self) -> None:
    ...
    def test_workflow_defaults_on_failure_object_form_inherited(self) -> None:
    ...
    def test_workflow_explicit_step_on_failure_wins_unchanged(self) -> None:
        workflow = WorkflowDefinition.model_validate(
            _base_workflow(
    ...
    def test_workflow_defaults_do_not_fill_loop_block_itself(self) -> None:
        workflow = WorkflowDefinition.model_validate(
            _base_workflow(
                defaults={"on_failure": "continue"},
                steps=[
    ...
    def test_workflow_defaults_reject_unknown_keys(self) -> None:
        with pytest.raises(ValidationError):
            WorkflowDefinition.model_validate(_base_workflow(defaults={"on_failure": "abort", "extra": True}))


class WorkflowModelSchemaTests:
    """Unit tests for JSON schema validation of workflow definitions and failure policies."""

    def test_workflow_schema_accepts_defaults_on_failure(self) -> None:
        result = WORKFLOW_VALIDATOR.validate(
            _base_workflow(defaults={"on_failure": {"action": "retry", "max_retries": 2}})
        )
        assert result.ok
        assert result.errors == []

    def test_workflow_schema_rejects_unknown_defaults_keys(self) -> None:
        result = WORKFLOW_VALIDATOR.validate(_base_workflow(defaults={"unknown": True}))
        assert not result.ok

    def test_workflow_schema_rejects_invalid_defaults_on_failure(self) -> None:
