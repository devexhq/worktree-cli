...
from worktree.core.db import BlueprintKind, RunsRepository, RunStatus, WorktreeDb
...
def _seed_paused_workflow(
    db: RunsRepository, git_fs: GitFileSystem, session_id: str, checkpoint: RunCheckpoint
) -> None:
...
class WorkflowResumeTests:
    """Unit tests for workflow pause/resume lifecycle and edge cases."""
    ...
    def test_resume_workflow_happy_path_skips_completed(
        ...
        git_fs.init_repo()
        _seed_paused_workflow(self.db.runs, git_fs, "wf-ok", _checkpoint())
        ...
        row = self.db.runs.get("wf-ok")
        assert row is not None
        assert row.status is RunStatus.COMPLETED

    def test_resume_workflow_wrong_status(self, git_fs: GitFileSystem) -> None:
        git_fs.init_repo()
        self.db.runs.create(
            session_id="wf-done",
    ...
    def test_resume_workflow_not_found(self, git_fs: GitFileSystem) -> None:
        git_fs.init_repo()
    ...
    def test_resume_workflow_missing_sandbox(self, git_fs: GitFileSystem) -> None:
        git_fs.init_repo()
    ...
    def test_resume_workflow_corrupt_checkpoint(self, git_fs: GitFileSystem) -> None:
        git_fs.init_repo()
        self.db.runs.create(
            session_id="wf-bad", blueprint_name="resume-demo", kind=BlueprintKind.WORKFLOW, branch_name="wt/resume"
        )
        self.db.runs.save_pause("wf-bad", "{nope", "paused")
    ...
    def test_resume_workflow_reprompts_pending_step(
        self, git_fs: GitFileSystem, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        git_fs.init_repo()
        _seed_paused_workflow(self.db.runs, git_fs, "wf-retry", _checkpoint())
