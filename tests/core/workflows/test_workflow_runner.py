"""Tests for the workflow iteration controller."""
...
def _workflow(
    *,
    ...
    object.__setattr__(
        wf,
        "_stop_when",
        set(stop_when) if stop_when is not None else {"trigger_passes", "unfixable", "user_abort"},
    )
...
def _run(
    *,
    ...
    def patch_fn(**kwargs):
        ...
        if not patch_queue:
            return PatchApplyResult(status=PatchApplyStatus.APPLIED, touched_files=["a.py"])
    ...
    eff_max_attempts = (
        caller_max_attempts if caller_max_attempts is not None else getattr(workflow, "_max_attempts", None)
    )
    eff_require = (
...
class ResolveMaxAttemptsTests:
    def test_workflow_value_clamped_by_hard_limit(self) -> None:
        cfg = _config(max_attempts_hard_limit=4)
        workflow = _workflow(max_attempts=10)
        assert resolve_max_attempts(workflow=workflow, config=cfg) == 4
    ...
    def test_caller_override(self) -> None:
        ...
        assert resolve_max_attempts(workflow=workflow, config=cfg, caller_max_attempts=7) == 7
...
class RunWorkflowIterationTests:
    ...
    def test_approval_rejected(self, sandbox: Path) -> None:
        result, _, _ = _run(
            sandbox=sandbox,
            workflow=_workflow(max_attempts=1, require_before_apply=True),
            triggers=[_trigger(TriggerRunStatus.FAILED)],
            agent_responses=[
                AgentResponse(
                    status=AgentResponseStatus.PROPOSED_PATCH,
    ...
    def test_approval_callback_missing(self, sandbox: Path) -> None:
        result, _, _ = _run(
            sandbox=sandbox,
            workflow=_workflow(max_attempts=2, require_before_apply=True),
            triggers=[_trigger(TriggerRunStatus.FAILED)],
            agent_responses=[
                AgentResponse(
                    status=AgentResponseStatus.PROPOSED_PATCH,
    ...
    def test_hard_limit_clamp_on_run(self, sandbox: Path) -> None:
        result, agent, _ = _run(
            sandbox=sandbox,
            workflow=_workflow(max_attempts=50),
    ...
    def test_cleanup_on_pass_when_auto_clean(self, sandbox: Path) -> None:
        result, _, cleaned = _run(
            sandbox=sandbox,
            workflow=_workflow(auto_clean=True, keep_on_failure=True),
            triggers=[_trigger(TriggerRunStatus.PASSED)],
        )
        assert result.status == WorkflowFinalStatus.PASSED
    ...
    def test_prompt_dump_writes_local_agent_input(self, sandbox: Path, tmp_path: Path) -> None:
    ...
    def test_prompt_dump_emits_event(self, sandbox: Path, tmp_path: Path) -> None:
        ...
        dump_payload = next(payload for name, payload in events if name == "agent_prompt_dumped")
        assert dump_payload["provider"] == "local"
        assert dump_payload["path"].endswith("wt-agent-prompt-sbx_test01-attempt-01.json")
    ...
    def test_direct_mutation_validation_failure_discards(self, sandbox: Path) -> None:
        ...
        spy: dict[str, list] = {}
        many_files_diff = "".join(f"diff --git a/f{i}.py b/f{i}.py\n" for i in range(11))
        result, _, _ = _run(
            sandbox=sandbox,
            workflow=_workflow(max_attempts=1),
            triggers=[_trigger(TriggerRunStatus.FAILED)],
...
class DirectMutationProviderTests:
    ...
    def test_gate_failure_from_adapter_does_not_double_discard(self, sandbox: Path) -> None:
...
class DiffArtifactTests:
    ...
    def test_git_diff_failure_records_warning_and_writes_empty_diff(self, tmp_path: Path) -> None:
        repo = tmp_path / "repo"
        repo.mkdir()
